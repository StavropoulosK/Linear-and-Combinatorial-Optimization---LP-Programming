...
def evaluate(coeff):
    ...
    for i in range(len(testSet)):
        ...
        if(g_value>=1):response='Φιλανδικό'
        elif(g_value>=0): response='Φιλανδικό μάλλον'
        elif(g_value<=-1): response='Σουηδικό'            
        elif(g_value<=0): response='Σουηδικό μάλλον'   

        print("όνομα αρχείου=%s g(fd)=%.2f, αποτέλεσμα: %s " % (file_name[i],g_value,response))


print('\n\nΑντικειμενική συνάρτηση με νόρμα 1\n\n')
coeff_first_norm=solve_first_norm()
evaluate(coeff_first_norm)


print('\n\nΑντικειμενική συνάρτηση με νόρμα άπειρο\n\n')
